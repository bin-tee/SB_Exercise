...
function hasOddNumber(arr) {
    const newArr = arr.some(function (num) {
        return num % 2 !== 0;
    });
    return newArr;
}
...
function hasAZero(num) {
    // convert num to string, split string into array
    const number = num.toString().split('').some(function (digit) {
        return digit === '0';
    });
    return number;
}
...
function hasOnlyOddNumbers(arr) {
    const newArr = arr.every(function (num) {
        return num % 2 !== 0;
    });
    return newArr;
}
...
function hasNoDuplicates(arr) {
    const newArr = arr.every(function (num) {
        /* check if first & last index of current num are the same
        if they're the same, there's no duplicate*/
...
function hasCertainKey(arr, key) {
    const object = arr.every(function (obj) {
        return key in obj;
    });
    return object;
}
